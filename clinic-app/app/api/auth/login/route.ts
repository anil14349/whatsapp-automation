...
export async function POST(request: Request) {
    ...
    if (body.role !== "owner" && !body.clinicId) {
        return NextResponse.json({ error: "Clinic is required" }, { status: 400 });
    }

    const payload =
        body.role === "doctor"
            ? { email: body.email, pin: body.secret, clinicId: body.clinicId }
            : body.role === "receptionist"
                ? { email: body.email, password: body.secret, clinicId: body.clinicId }
                : { email: body.email, password: body.secret, clinicId: body.clinicId || undefined };
    ...
    const session = {
        ...
        clinicId: profile.clinicId ?? profile.clinic?.id ?? body.clinicId
    };
