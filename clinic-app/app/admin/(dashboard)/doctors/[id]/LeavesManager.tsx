...
function CancelButton({ doctorId, leaveDate }: { doctorId: string; leaveDate: string }) {
  return (
    <form action={cancelLeaveAction.bind(null, doctorId, leaveDate)}>
...
export function LeavesManager({
  ...
  const boundAction = addLeaveAction.bind(null, doctorId);
        ...
        <ul className="divide-y divide-slate-100">
          {leaves.map((leave) => (
            <li key={leave.id} className="flex items-center justify-between py-2 text-sm">
              ...
              <CancelButton doctorId={doctorId} leaveDate={leave.leave_date} />
            </li>
          ))}
        </ul>
      )}
