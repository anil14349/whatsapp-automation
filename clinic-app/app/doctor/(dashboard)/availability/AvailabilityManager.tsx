...
function RemoveButton({ sessionId }: { sessionId: string }) {
  return (
    <form action={removeAvailabilityAction.bind(null, sessionId)}>
...
export function AvailabilityManager({ sessions }: { sessions: DoctorAvailability[] }) {
  const [state, formAction] = useFormState(addAvailabilityAction, initialState);
                ...
                <ul className="space-y-1">
                  {daySessions.map((session) => (
                    <li key={session.id} className="flex items-center gap-3">
                      ...
                      <RemoveButton sessionId={session.id} />
                    </li>
                  ))}
                </ul>
              )}
